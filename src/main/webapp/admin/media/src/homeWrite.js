	...
	return {
		...
		initEvent: function(){
			...
			self.textarea.bind('input propertychange', CommonUtils.throttle(function(){
				...
				self.ajaxEdit("updateHome",{title: textareaVal});
			}, 1000));
			
			// 富文本框的内容改变事件
			self.ue.addListener("contentChange",CommonUtils.throttle(function(){
				var contentVal = self.ue.getContent();
				self.ajaxEdit("updateHome", {content: contentVal});
			}, 2000));
		},
		initData: function(){
			var self = this;
			self.ue.addListener("ready", function () {
				self.ajaxEdit("queryHome",{});
	    }); 
		},
		ajaxEdit: function(path, obj){
			var self = this;
			$.ajax({
				type:"post",
				url: self.baseurl+"/home/"+path+"/"+self.articleId,
				async:true,
				data: obj,
